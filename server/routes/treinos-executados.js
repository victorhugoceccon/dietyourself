...
router.get('/recent', authenticate, async (req, res) => {
  try {
    ...
    if (!hasAnyRole(user, ['PACIENTE', 'ADMIN'])) {
      return res.status(403).json({ error: 'Apenas pacientes podem acessar treinos recentes' })
    }
    ...
    console.error('Erro ao listar treinos recentes:', error)
    res.status(500).json({ error: 'Erro ao listar treinos recentes' })
  }
})
